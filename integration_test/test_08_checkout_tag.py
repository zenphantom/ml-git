...
class CheckoutTagAcceptanceTests(unittest.TestCase):
    CHECKOUT_COMMAND = 'ml-git dataset checkout computer-vision__images__dataset-ex__12'
    UPDATE_COMMAND = 'ml-git dataset update'
    ...
    def setUp_test(self):
        ...
        self.assertIn(messages[20] % (os.path.join(ML_GIT_DIR, "dataset", "metadata")),
                      check_output(self.UPDATE_COMMAND))

    def test_01_checkout_tag(self):
        self.setUp_test()

        check_output(self.CHECKOUT_COMMAND)
    ...
    def test_02_checkout_with_group_sample(self):
        ...
        self.assertIn("", check_output(self.CHECKOUT_COMMAND + " --group-sample=2:4 --seed=5"))

    def test_03_group_sample_with_amount_parameter_greater_than_group_size(self):
        self.setUp_test()

        self.assertIn(messages[21], check_output(self.CHECKOUT_COMMAND + " --group-sample=4:2 --seed=5"))

    def test_04_group_sample_with_amount_parameter_equal_to_group_size(self):
        self.setUp_test()

        self.assertIn(messages[21], check_output(self.CHECKOUT_COMMAND + " --group-sample=2:2 --seed=5"))

    def test_05_group_sample_with_group_size_parameter_greater_than_list_size(self):
        self.setUp_test()

        self.assertIn(messages[22], check_output(self.CHECKOUT_COMMAND + " --group-sample=2:30 --seed=5"))

    def test_06_group_sample_with_group_size_parameter_less_than_zero(self):
        self.setUp_test()
        self.assertIn(messages[41], check_output(self.CHECKOUT_COMMAND + " --group-sample=2:-3 --seed=5"))

    def test_07_checkout_with_range_sample(self):
        self.setUp_test()

        self.assertIn("", check_output(self.CHECKOUT_COMMAND + " --range-sample=2:4:1"))

    def test_08_range_sample_with_start_parameter_greater_than_stop(self):
        self.setUp_test()

        self.assertIn(messages[23], check_output(self.CHECKOUT_COMMAND + " --range-sample=4:2:1"))

    def test_09_range_sample_with_start_parameter_less_than_zero(self):
        self.setUp_test()
        self.assertIn(messages[42], check_output(self.CHECKOUT_COMMAND + " --range-sample=-3:2:1"))

    def test_10_range_sample_with_step_parameter_greater_than_stop_parameter(self):
        self.setUp_test()
        self.assertIn(messages[26], check_output(self.CHECKOUT_COMMAND + " --range-sample=1:3:4"))

    def test_11_range_sample_with_start_parameter_equal_to_stop(self):
        self.setUp_test()
        self.assertIn(messages[23], check_output(self.CHECKOUT_COMMAND + " --range-sample=2:2:1"))

    def test_12_range_sample_with_stop_parameter_greater_than_file_list_size(self):
        self.setUp_test()
        self.assertIn(messages[24], check_output(self.CHECKOUT_COMMAND + " --range-sample=2:30:1"))

    def test_13_checkout_with_random_sample(self):
        self.setUp_test()

        self.assertIn('', check_output(self.CHECKOUT_COMMAND + " --random-sample=2:3 --seed=3"))

    def test_14_random_sample_with_frequency_less_or_equal_zero(self):
        self.setUp_test()

        self.assertIn(messages[40], check_output(self.CHECKOUT_COMMAND + " --random-sample=2:-2 --seed=3"))

    def test_15_random_sample_with_amount_parameter_greater_than_frequency(self):
        self.setUp_test()

        self.assertIn(messages[30], check_output(self.CHECKOUT_COMMAND + " --random-sample=4:2 --seed=3"))

    def test_16_random_sample_with_frequency_greater_or_equal_list_size(self):
        self.setUp_test()

        self.assertIn(messages[31], check_output(self.CHECKOUT_COMMAND + " --random-sample=2:10 --seed=3"))

    def test_17_random_sample_with_frequency_equal_zero(self):
        self.setUp_test()

        self.assertIn(messages[29], check_output(self.CHECKOUT_COMMAND + " --random-sample=2:0 --seed=3"))

    def test_18_group_sample_with_group_size_parameter_equal_zero(self):
        self.setUp_test()

        self.assertIn(messages[28], check_output(self.CHECKOUT_COMMAND + " --group-sample=1:0 --seed=5"))

    def test_19_group_sample_with_amount_parameter_equal_zero(self):
        self.setUp_test()

        self.assertIn(messages[43], check_output(self.CHECKOUT_COMMAND + " --group-sample=0:1 --seed=5"))
