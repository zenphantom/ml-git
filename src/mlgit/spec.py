...
def get_spec_file_dir(spec_file, repotype='dataset'):
	dir1 = os.path.join(repotype, spec_file)
	return dir1
...
def increment_version_in_spec(spec_file, repotype='dataset'):
	...
	if os.path.exists(spec_file):
		version1 = incr_version(spec_file, repotype)
		if version1 is not -1:
			...
			log.error(
				...
				"%s\n" % spec_file, class_name=ML_GIT_PROJECT_NAME)
			return False
	else:
		log.error(
