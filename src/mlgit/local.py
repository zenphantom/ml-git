...
class LocalRepository(MultihashFS):
	...
	def checkout(self, cachepath, metadatapath, objectpath, wspath, tag, samples, bare=False):
		...
		if not bare:
			...
			self._update_index_bare_mode(lkey, fidex, wspath, objfiles)
	...
	def _update_index_bare_mode(self, lkey, fidex, wspath, objfiles):
		for i in range(0, len(lkey), 20):
			lkeys = lkey[i:i+20]
			for key in lkeys:
				[fidex.update_full_index(file, wspath, Status.u.name, key) for file in objfiles[key]]
	...
	def status(self, spec, log_errors=True):
		...
		bare_mode = os.path.exists(os.path.join(index_metadatapath, spec, "bare"))
