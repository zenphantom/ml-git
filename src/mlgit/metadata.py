...
class Metadata(MetadataManager):
	...
	def is_version_type_not_number(self, index_path):
		...
		if type (metadata[self.__repotype]["version"]) == int:
			...
			log.error("Version %s must be a number" % (metadata[self.__repotype]["version"]), METADATA_CLASS_NAME)
			return True
	...
	def commit_metadata(self, index_path, tags):
		...
		log.debug("Metadata path [%s]" % full_metadata_path, METADATA_CLASS_NAME)
		...
		if ret is False:
			log.info("No files to commit for [%s]" % self._spec, METADATA_CLASS_NAME)
			return None, None
		...
		if len(tags) > 0:
			log.error("Tag [%s] already exists in the ml-git repository. "
					"Consider using --bumpversion parameter to increment the version number for your dataset." % tag, METADATA_CLASS_NAME)
			for t in tags: log.error("\t%s" % (t))
			return None, None

		# generates a commit message
		msg = self.metadata_message(metadata)
		log.debug("Commit message [%s]" % msg, METADATA_CLASS_NAME)
	...
	def metadata_subpath(self, metadata):
		...
		log.debug("Dataset path: %s" % path, METADATA_CLASS_NAME)
		return path

	def _full_metadata_path(self, spec_file):
		log.debug("Getting subpath from categories in specfile [%s]" % spec_file, METADATA_CLASS_NAME)

		metadata = yaml_load(spec_file)
		if metadata == {}:
			log.error("The entity name passed it's wrong. Please check again", METADATA_CLASS_NAME)
			return None, None, None
		categories_path = self.metadata_subpath(metadata)
		if categories_path is None:
			log.error("You must place at least one category in the entity .spec file", METADATA_CLASS_NAME)
			return None, None, None
	...
	def __commit_manifest(self, fullmetadatapath, index_path):
		...
		if os.path.exists(idxpath) == False:
			log.error("Not manifest file found in [%s]" % idxpath, METADATA_CLASS_NAME)
			return False
	...
	def __commit_metadata(self, fullmetadatapath, index_path, metadata, specs):
		...
		log.debug("Commit spec [%s] to ml-git metadata" % self._spec, METADATA_CLASS_NAME)
		...
		if os.path.exists(src_readme):
			...
			try:
				...
				log.error("Could not find file README.md. Entity repository must have README.md file", METADATA_CLASS_NAME)
				raise e
		...
		if "dataset" in specs and self.__repotype in ["labels", "model"]:
			...
			tag, sha = r.head()
			if tag is not None:
				log.info("Associate dataset [%s]-[%s] to the %s." % (dspec, tag, self.__repotype), LOCAL_REPOSITORY_CLASS_NAME)
		...
		if "labels" in specs and self.__repotype in ["model"]:
			...
			tag, sha = r.head()
			if tag is not None:
				log.info("Associate labels [%s]-[%s] to the %s." % (lspec, tag, self.__repotype), LOCAL_REPOSITORY_CLASS_NAME)
	...
	def metadata_tag(self, metadata):
		...
		log.debug("New tag created [%s]" % tag, METADATA_CLASS_NAME)
		return tag
