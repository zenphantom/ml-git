...
def initialize_data_set_repository(data_set_source_dir, name, version, labels, storage_type):
    ...
    if cwd not in data_set_path:
        ...
        create_metadata_files(cwd, data_set_path, name, version, labels, storage_type)


def create_metadata_files(cwd, data_set_path, name, version, labels, storage_type):
    relative_data_set_source = Path(data_set_path).relative_to(cwd).as_posix()
    ml_git_repository.create_repository_configuration_file(cwd, name, version, labels, storage_type,
